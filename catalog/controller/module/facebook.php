	...
	protected function index($setting) {
		...
		if(!$this->customer->isLogged()){
			...
			if($this->config->get('facebook_button_' . $this->config->get('config_language_id'))){
				$data['facebook_button'] = html_entity_decode($this->config->get('facebook_button_' . $this->config->get('config_language_id')));
			}

			else $data['facebook_button'] = $this->language->get('heading_title');
