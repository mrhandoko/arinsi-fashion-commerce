...
class ControllerModuleSorelatedtabs extends Controller {
	public function index($setting) {
		...
		$data['tag_id'] 			= 'so_related_tabs_'.$data['moduleid'];
		...
		if($is_ajax && isset($_POST['is_ajax_related_tabs']) && $_POST['is_ajax_related_tabs']){
			...
			$tag_id						= 'so_related_tabs_'.$_POST['lbmoduleid'];
			...
			$result = new stdClass();
			ob_start();
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/so_related_tabs/default.tpl')) {
				include(DIR_TEMPLATE .$this->config->get('config_template') . '/template/module/so_related_tabs/default_items.tpl');
			} else {
				include(DIR_TEMPLATE .'default/template/module/so_related_tabs/default_items.tpl');
			}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/so_related_tabs/default.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/module/so_related_tabs/default.tpl', $data);
		} else {
			return $this->load->view('default/template/module/so_related_tabs/default.tpl', $data);
		}
	}
	public function getListCategoriesFilter($setting)
	{
		...
		foreach ($cats as $cat) {
			...
			$cat['count'] = $this->model_module_so_related_tabs->getTotalProducts_listing_tabs($filter_data);
			if($cat['count'] > 0){
				$_cats[] = $cat;
			}
	...
	public function getListArticlesFilter($setting)
	{
		...
		foreach ($filters as $filter) {
			...
			$aritles['count'] = $this->model_module_so_related_tabs->getTotalProducts_listing_tabs($filter_data);
	...
	public function getCategoryson($category_id, $setting)
	{
		...
		if($setting['child_category'] ==1)
		{
			...
			for($i=1; $i<= (int)$setting['category_depth'];$i++)
			{
				...
				$categoryss = $this->model_module_so_related_tabs->getCategories_son_listing_tabs($filter_data);
	...
	public function getProducts($category_id_list,$setting)
	{
		...
		if ($cat['count'] > 0) 
		{
			$products_arr = $this->model_module_so_related_tabs->getProducts_listing_tabs($filter_data);
			foreach($products_arr as $product_info)
			{
				...
				if ($product_info['image'] && $setting['product_get_image_data']) {
					...
					$url = file_exists("image/so_related_tabs/images/".$setting['product_placeholder_path']);
				if ($url) {
					$image_name = "so_related_tabs/images/".$setting['product_placeholder_path'];
				} else {
