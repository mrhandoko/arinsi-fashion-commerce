<?php
class ControllerModuleSorelatedtabs extends Controller {
	private $error = array();
	private $data = array();
	public function index() {

		// Load language
		$this->load->language('module/so_related_tabs');
		...
		$this->load->model('module/so_related_tabs');
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('module/so_related_tabs', 'token=' . $this->session->data['token'], 'SSL'));
		}
	// Get Module Id new 
		$moduleid_new= $this->model_module_so_related_tabs->getModuleId();
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
				$module_id = $moduleid_new[0]['Auto_increment'];
				$this->model_extension_module->addModule('so_related_tabs', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('module/so_related_tabs', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/so_related_tabs', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/so_related_tabs', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}

		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$moduleid_new= $this->model_module_so_related_tabs->getModuleId(); // Get module id
		...
		$data['moduletabs'] = $this->model_extension_module->getModulesByCode( 'so_related_tabs' );
		$data['link'] = $this->url->link('module/so_related_tabs', 'token=' . $this->session->data['token'] . '', 'SSL');
		//--------------------------------Load Data -------------------------------------------
		...
		$this->response->setOutput($this->load->view('module/so_related_tabs.tpl', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/so_related_tabs', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/so_related_tabs', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/so_related_tabs')) {
	...
	public function autocomplete_category() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_module_so_related_tabs->getCategories($filter_data);
